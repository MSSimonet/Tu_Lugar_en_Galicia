...
const CATEGORIA_ICON: Record<App['categoria'], React.ReactNode> = {
  identidad:  <IconIdentidad />,
  salud:      <IconSalud />,
  transporte: <IconTransporte />,
  hogar:      <IconHogar />,
  burocracia: <IconBurocracia />,
}
...
export function AppCardPedraEOuro({ app }: Props) {
  return (
    <article
      className="flex flex-col gap-3 rounded p-5 transition-brand hover:shadow-md"
      style={{
        borderRadius: '4px',
    ...
    >
      ...
      <div className="flex items-start gap-3">
        <div
          ...
          style={{
            borderRadius: '4px',
            backgroundColor: 'color-mix(in srgb, var(--po-ouro) 12%, transparent)',
            color: 'var(--po-ouro-text)',
          }}
        >
          {CATEGORIA_ICON[app.categoria]}
        </div>
        <h3
          className="text-base font-semibold leading-snug"
          style={{ fontFamily: 'var(--font-playfair)', color: 'var(--po-pedra)' }}
        >
          {app.nombre}
        </h3>
      </div>
      ...
      <div
        ...
        style={{
          borderRadius: '4px',
          backgroundColor: 'color-mix(in srgb, var(--po-ouro) 10%, transparent)',
          color: 'var(--po-ouro-text)',
        }}
      >
        <span className="mt-px shrink-0 font-bold" aria-hidden="true">▸</span>
        <p style={{ fontFamily: 'var(--font-lato)' }} className="leading-snug">{app.paraQue}</p>
      </div>
        ...
        <div className="mt-auto flex flex-wrap gap-2 pt-1">
          {app.links.ios && (
            <a
              ...
              className="po-link-descarga inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium transition-brand"
              style={{
